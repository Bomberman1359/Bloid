...
class BBrobot:
    ...
    def __init__(self, gpio_pins):  # Receive a list of GPIO pins
        # Prepare the servos
        self.s1 = cs.RS304MD(gpio_pins[0])
        self.s2 = cs.RS304MD(gpio_pins[1])
        self.s3 = cs.RS304MD(gpio_pins[2])
    ...
    def kinema_inv(self, n, Pz):
        L = self.L
        # Derive the height Pmz at the servo reference (reverse sign with Pmz)
        A = (L[0]+L[1])/Pz
